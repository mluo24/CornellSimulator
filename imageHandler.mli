(* change this so this has a type t so that this can truly be some sort of
   module? *)
...
(** [get_entire_image filename] takes a [filename] and converts it to an
    image.

    Requires: [filename] is a valid PNG image. *)
val get_entire_image : string -> Images.t

(** [make_transparent img] takes an image and turns the black pixels to a
    transparently defined color in the [Graphics] module. *)
val make_transparent : Graphics.image -> Graphics.image

(** [get_tileset_part x y w h image] takes a sub image of [image] starting at
    position ([x], [y]) of size [w] * [h]. *)
