...
let make_transparent img = 
  ...
  let newimg = Array.map (fun x -> Array.map (fun y -> if y = 0 then Graphics.transp else y) x) arr in
...
(* let main () =
  try
    Graphics.open_graph "";
    let img = Images.sub (Png.load_as_rgb24 "assets/Terrain.png" []) 16 0 16 32 in
    let g = Graphic_image.of_image img |> make_transparent in
