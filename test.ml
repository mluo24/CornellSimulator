...
(** [cmp_set_like_lists lst1 lst2] compares two lists to see whether
    ...
    let world_test_arr_from_txt name filename expected_output =
      name >:: fun _ ->
        assert_equal expected_output (arr_from_txt filename) ~printer:(string_of_array string_of_int)
    ...
    let world_tests = [
      world_test_arr_from_txt "empty file gives empty array" "empty.txt" (Array.make 0 0)
    ]
