...
(** A [Comparable] is a value that can be compared. The comparison is a
    total order on the values. *)
module type Comparable = sig
  (** The type of comparable values. *)
  type t

  (** [compare t1 t2] is [LT] if [t1] is less than [t2], [EQ] if [t1] is
      equal to [t2], or [GT] if [t1] is greater than [t2]. *)
  val compare : t -> t -> order
end

(** A [Formattable] is a value that can be formatted with the toplevel
    printer. *)
module type Formattable = sig
  ...
  (** [format] is a printing function suitable for use with the
      ...
      representation of a value of type [t] on the given formatter. *)
  val format : Format.formatter -> t -> unit

  (* val stringnigy : t -> string *)
end

(** A module that matches [KeySig] is suitable for use as the type of
    keys in a [Dictionary]. Keys must be comparable and formattable. *)
...
(** A module that matches [ValueSig] is suitable for use as the type of
    values in a [Dictionary]. Values must be formattable. *)
...
module type Dictionary = sig
  (** [Key] is a module representing the type of keys in the dictionary
      and functions on them. *)
  module Key : KeySig

  (** [Value] is a module representing the type of values in the
      dictionary and functions on them. *)
  ...
  (** [rep_ok d] returns [d] if [d] satisfies its representation
      invariants. It's unusual for a data abstraction to expose this
      function to its clients, but we do so here to ensure that you
      implement it. Raises: [Failure] with an unspecified error message
  ...
  (** [find k d] is [Some v] if [k] is bound to [v] in [d]; or if [k] is
      not bound, then it is [None]. *)
  val find : key -> t -> value option

  (** [remove k d] contains all the bindings of [d] except a binding for
      [k]. If [k] is not bound in [d], then [remove] returns a
      dictionary with the same bindings as [d]. *)
  val remove : key -> t -> t
  ...
  (** [to_list d] is an association list containing the same bindings as
      [d]. The elements in the list are in order from the least key to
      the greatest. There are no duplicate keys in the list. *)
...
(** A [DictionaryMaker] is a functor that makes a [Dictionary] out of
