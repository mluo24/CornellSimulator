...
module type Formattable = sig
  ...
  (** [format] is a printing function suitable for use with the toplevel's
      [#install_printer] directive. It outputs a textual representation of a
      value of type [t] on the given formatter. *)
...
(** A module that matches [KeySig] is suitable for use as the type of keys in
    a [Dictionary]. Keys must be comparable and formattable. *)
...
(** A module that matches [ValueSig] is suitable for use as the type of values
    in a [Dictionary]. Values must be formattable. *)
...
module type Dictionary = sig
  ...
  (** [Value] is a module representing the type of values in the dictionary
  ...
  (** [rep_ok d] returns [d] if [d] satisfies its representation invariants.
      It's unusual for a data abstraction to expose this function to its
      clients, but we do so here to ensure that you implement it. Raises:
      [Failure] with an unspecified error message if [d] does not satisfy its
  ...
  (** [remove k d] contains all the bindings of [d] except a binding for [k].
      If [k] is not bound in [d], then [remove] returns a dictionary with the
      same bindings as [d]. *)
  val remove : key -> t -> t

  val op_val : key -> (value -> value -> value) -> value -> t -> t

  (** [fold f init d] is [f kn vn (f ... (f k1 v1 init) ...)], if [d] binds
      [ki] to [vi]. Bindings are processed in order from least to greatest,
      where [k1] is the least key and [kn] is the greatest. *)
  val fold : (key -> value -> 'acc -> 'acc) -> 'acc -> t -> 'acc

  (** [to_list d] is an association list containing the same bindings as [d].
      The elements in the list are in order from the least key to the
      greatest. There are no duplicate keys in the list. *)
  val to_list : t -> (key * value) list

  (** [format] is a printing function suitable for use with the toplevel's
      [#install_printer] directive. It outputs a textual representation of a
      dictionary on the given formatter. *)
  val format : Format.formatter -> t -> unit
end
